package automationcraft.testcreation.jetsmartGrupo5.steps;

import io.cucumber.java.en.And;
import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;

public class StepsModuloVueloAltoG5 {
    @Given("soy el usuario e ingreso a la pagina {string}")
    public void soy_el_usuario_e_ingreso_a_la_pagina(String string){

    }

    @When("ingreso al formulario de vuelo")
    public void ingreso_al_formulario_de_vuelo(String string){

    }

    @And("selecciono {string} en el campo Origen")
    public void selecciono_en_el_campo_Origen(String string){

    }

    @And("selecciono {string} en el campo Destino")
    public void selecciono_en_el_campo_Destino(String string){

    }

    @And("Seleciono el Checkbox de Ida")
    public void Seleciono_el_Checkbox_de_Ida(String string){

    }

    @And("Seleciona la primera fecha disponible ida")
    public void Seleciona_la_primera_fecha_disponible_ida(String string){

    }

    @And("Seleciono un Adulto")
    public void Seleciono_un_Adulto(String string){

    }

    @And("Seleciono dos ninos")
    public void Seleciono_dos_ninos(String string){

    }

    @And("presiono BuscarSmart")
    public void presiono_BuscarSmart(String string){

    }

    @And("cierro la ventana club descuento")
    public void cierro_la_ventana_club_descuento(String string){

    }

    @And("Seleciono el boton {string}")
    public void Seleciono_el_boton(String string){

    }


    @And("Selecion la segunda fecha de vuelo disponible del proximo mes")
    public void Selecion_la_segunda_fecha_de_vuelo_disponible_del_proximo_mes(String string){

    }

    @And("Presiono el boton continuar")
    public void Presiono_el_boton_continuar(String string){

    }


    @And("Seleciono el Pack de vuelo mas barato")
    public void Seleciono_el_Pack_de_vuelo_mas_barato(String string){

    }

    @And("fijo precio por 24hrs")
    public void fijo_precio_por_24hrs(String string){

    }

    @And("Clickear en Continuar")
    public void Clickear_en_Continuar(String string){

    }

    @And("Clickear en pagar ahora")
    public void Clickear_en_pagar_ahora(String string){

    }

    @Then("Se muestra en la pantalla de medios de pago, la fecha cambiada de IDA con la nueva fecha de vuelo más barata")
    public void Se_muestra_en_la_pantalla_de_medios_de_pago_la_fecha_cambiada_de_IDA_con_la_nueva_fecha_de_vuelo_más_barata(){

    }


}
